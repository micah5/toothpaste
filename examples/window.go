...
func main() {
	...
	//))
	frame := toothpaste.NewNode(toothpaste.NewFace3D(
		0.018708, 0.407143, 0.005000, 0.075000, 0.407143, 0.037500, 0.075000, 0.155714, 0.037500, 0.018708, 0.155714, 0.005000,
	))
	frameSquare2D := toothpaste.Square(0.8, 0.8)
	frameSquare2D.Translate(0.1, 0.1)
	frame.AddHoles(frameSquare2D)

	println(frame.Outer.Normal().X, frame.Outer.Normal().Y, frame.Outer.Normal().Z)
	outside := frame.ExtrudeFlip(0.0125)
	fmt.Println(outside.Outer.Vertices)
